...
class PkpassParser(context: Context, uri: Uri?) {
    ...
    init {
        ...
        val inputStream = mContext.contentResolver.openInputStream(uri) ?: throw IOException(mContext.getString(R.string.errorReadingFile))
    ...
    private fun parsePassJSON(jsonObject: JSONObject, locale: String?) {
        ...
        try {
            headerColor = Color.parseColor(jsonObject.getString("backgroundColor"))
        } catch (ignored: JSONException) {
